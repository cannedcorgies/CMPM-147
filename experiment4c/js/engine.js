...
function playBeep() {
  ...
  // Set the type of oscillator (sine wave)
  oscillator.type = 'sine';

  // Connect the oscillator to the AudioContext's destination (speakers)
  oscillator.connect(audioContext.destination);

  // Start the oscillator
  oscillator.start();

  // Stop the oscillator after 0.1 second to generate a short beep
  setTimeout(() => {
    oscillator.stop();
  }, 100);
}
...
function setup() {
  ...
  createP("Minesweeper. Arrow keys scroll. Clicking selects tile. Enter a new world key to start over.").parent("container");
  createP("Hover over safe tile to listen for mines. Faster beeping means more mines around.").parent("container");
...
function mouseClicked() {
  ...
  if (!gameOver) {
    ...
    if (window.p3_tileClicked && !gameOver) {
      gameOver = window.p3_tileClicked(world_pos[0], world_pos[1]);
    }
...
function draw() {
  ...
  // Check if one second has passed since the last beep
  if (millis() - lastBeepTime >= 1000 / beepsPerSecond) {
    // Play the beep sound
    playBeep();
    // Update the last beep time
    lastBeepTime = millis();
  }
  ...
  if (gameOver) {
    stroke(255, 255, 255);
    fill(255, 0, 0);
    text("G A M E  O V E R", width/3 * 2, height/3 * 2);
  }
  
  text(score, width/3 * 2 + 60, height/3 * 2 + 20);
